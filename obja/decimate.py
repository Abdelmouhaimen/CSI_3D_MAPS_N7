...
class Decimater(obja.Model):
    ...
    def remove_vertex_and_retriangulate(self, vertex_index):
        ...
        if not self.test_manifold(new_faces , cycle):
            print("Erreur : arête non-manifold détectée")
    ...
    def remove_vertex_and_retriangulate(self, vertex_index):
        ...
        if not self.test_manifold(new_faces , cycle):
            print("arête non-manifold détectée apres la triangulation")
    ...
    def subdivide_face(self, face):
        """
        Subdivides a face into four smaller faces by splitting each edge.
        :param face: The face to subdivide.
        :return: A list of new faces created from subdivision.
        """
        # Get the indices of the face's vertices
        a, b, c = face.a, face.b, face.c

        # Compute midpoints of each edge and add them to the vertex list
        ab_mid = self.get_or_create_midpoint(a, b)
        bc_mid = self.get_or_create_midpoint(b, c)
        ca_mid = self.get_or_create_midpoint(c, a)

        # Create new faces from the subdivided triangle
        new_faces = [
            Face(a, ab_mid, ca_mid),
            Face(ab_mid, b, bc_mid),
            Face(ca_mid, bc_mid, c),
            Face(ab_mid, bc_mid, ca_mid)
        ]

        return new_faces

    def get_or_create_midpoint(self, v1_idx, v2_idx):
        """
        Returns the index of the midpoint between two vertices, creating it if necessary.
        """
        ...
        if edge in self.edge_midpoints:
            ...
            #self.vertices_mapped_back.append(self.Map_back_midpoint(midpoint)) # MMMMMMMMMMMapping back the midpoint to the original domain
            midpoint_idx = len(self.vertices) - 1
            self.edge_midpoints[edge] = midpoint_idx
            return midpoint_idx
    ####################################################################
